...
dfo_synoptic_grid <- dplyr::select(
  synoptic_grid, survey, lat,
...
afsc_ebs_grid <- as.data.frame(eastern_bering_sea_grid) %>%
  ...
  dplyr::mutate(
    survey = "Eastern Bering Sea Crab/Groundfish Bottom Trawl Survey",
    survey_domain_year = as.numeric(format(Sys.Date(), "%Y")),
    depth_m = NA
  )
...
afsc_slope_grid <- as.data.frame(bering_sea_slope_grid) %>%
  ...
  dplyr::select(lat, lon, area_km2) %>%
  dplyr::mutate(
    survey = "Eastern Bering Sea Slope Bottom Trawl Survey",
    survey_domain_year = as.numeric(format(Sys.Date(), "%Y")),
    depth_m = NA
  )
# usethis::use_data(afsc_ebs_grid, overwrite = TRUE)
...
afsc_ai_grid <- as.data.frame(aleutian_islands_grid) %>%
  ...
  dplyr::select(lat, lon, area_km2) %>%
  dplyr::mutate(
    survey = "Aleutian Islands Bottom Trawl Survey",
    survey_domain_year = as.numeric(format(Sys.Date(), "%Y")),
    depth_m = NA
  )
...
afsc_nbs_grid <- as.data.frame(northern_bering_sea_grid) %>%
  ...
  dplyr::select(lat, lon, area_km2) %>%
  dplyr::mutate(
    survey = "Northern Bering Sea Crab/Groundfish Survey - Eastern Bering Sea Shelf Survey Extension",
    survey_domain_year = as.numeric(format(Sys.Date(), "%Y")),
    depth_m = NA
  )
...
afsc_grid <- rbind(
  afsc_nbs_grid, afsc_ebs_grid, afsc_slope_grid,
